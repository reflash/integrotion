...
export class AchievementNotionService implements IAchievementNotionService {
    constructor(private readonly client: Client, private readonly bot: Bot) {}

    public async getNewAchievements() {
        const achievements = await this.client.databases.query({
            database_id: process.env.ACHIEVEMENTS_NOTION_DATABASE!,
            filter: {
                and: [
