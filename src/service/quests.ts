...
import { NumberPropertyValue, SelectPropertyValue, DatePropertyValue, TitlePropertyValue, RichTextPropertyValue, RelationPropertyValue, MultiSelectPropertyValue, Page } from "@notionhq/client/build/src/api-types";
...
export class QuestService implements IQuestService {
    ...
    public async getQuestByName(name: string) {
        const pages = await this.client.databases.query({ database_id: process.env.QUEST_NOTION_DATABASE!, 
